from quick_start import *
from basic_usage import *
from advanced_usage.Saving import *
from advanced_usage.Loading import *
from advanced_usage.Comparison import *

if __name__ == '__main__':
    # # Please uncomment the example you want to try out

    # # Quick Start
    set_license_from_file.run()  
    # set_license_from_stream.run()
    # set_metered_license.run()
    hello_world.run()

    # # Basic Usage
    # compare_documents_from_path.run()
    # compare_documents_from_stream.run()
    # compare_documents_protected_path.run()
    # compare_documents_protected_stream.run()
    # compare_multiple_pdf_documents.run()
    # get_document_info_path.run()
    # get_document_info_stream.run()
    # get_supported_formats.run()

    # # Advanced usage/Saving
    # set_document_metadata_source.run()
    # set_document_metadata_target.run()
    # set_password_for_resultant_document.run()
    
    # # Advanced usage/Loading
    # load_document_from_local_disc.run()
    # load_document_from_stream.run()
    # load_text_from_string.run()

    # # Advanced usage/Comparison
    # compare_multiple_documents_stream.run()
    # compare_multiple_documents_settings_path.run()
    # compare_multiple_documents_settings_stream.run()
    # compare_multiple_documents_path.compare_multiple_words_documents()
    # compare_multiple_documents_path.compare_multiple_pdf_documents()
    # compare_multiple_documents_path.compare_multiple_txt_documents()
    # compare_folders.compare_folder_save_as_txt()
    # compare_folders.compare_folder_save_as_html()
    # compare_multiple_documents_protected_stream.compare_multiple_protected_words_documents()
    # compare_multiple_documents_protected_path.compare_multiple_protected_words_documents()